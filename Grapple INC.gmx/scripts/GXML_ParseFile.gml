...
show_debug_message("Parsing file " + FileName + " at time: " + string(get_timer() / 1000000));
...
while(!file_text_eof(FileRead)) {
    ...
    // Ugly solution to get better line reading speed.
    if(CurrentState == GXML_States.Content && string_count("<", Line) == 0) {
        for(var SkipCharIt = 1; SkipCharIt <= string_length(DataSkipChars); SkipCharIt++) {
            Line = string_replace_all(Line, string_char_at(DataSkipChars, SkipCharIt), "");
        }
...
show_debug_message("File parse complete at time: " + string(get_timer() / 1000000));
